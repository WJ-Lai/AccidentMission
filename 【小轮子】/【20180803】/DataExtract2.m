...
function [data_n_people, data_n_week] = DataExtract2(data_people, data_week, id_death, n)
    ...
    %针对week数据，直接筛选目标数据
    data_n_week = zeros(m_week,n);
    for i_week = 1:n
        %死亡人数
        data_n_week(:,i_week) = data_week(:,id_death(i_week));
    end
