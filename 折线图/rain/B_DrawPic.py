...
young = ax.scatter(x,Y_death, s=Y_accident, alpha=0.5)
middle= ax.scatter(x,M_death, s=M_accident, alpha=0.5)
old = ax.scatter(x,O_death, s=O_accident, alpha=0.5)

#设置坐标轴文字
ax.set_xlabel('Accident Type', fontsize=15)
ax.set_ylabel('Mortality Rate (%)', fontsize=15)
ax.set_title('Crash Involvement Pattern of Different Age Group on Rainy Day', fontsize=15)
#设置坐标轴最大最小值
plt.xlim(-1,22)
